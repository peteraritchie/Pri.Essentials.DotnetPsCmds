...
public partial class CreateXunitProjectCommandShould
	: CommandTestingBase<CreateXunitProjectCommand>
{
	private readonly IFileSystem spyFileSystem = Substitute.For<IFileSystem>();

	public CreateXunitProjectCommandShould()
	...
	private static CreateXunitProjectCommand InitializeSut(string projectDirectory, IShellExecutor executor, IFileSystem fileSystem)
	{
		return new CreateXunitProjectCommand(shellExecutor: executor,
	...
	void HaveCorrectCommand()
	{
		...
		Assert.Equal("xunit", match.Groups[2].Value);
	}
